...
public class BusinessUser extends User {
	 ...
	 * @param userPw 고객비밀번호
	 * @param userName 고객이름
	 * @param phoneNumber 고객연락처
	 * @param categoryCode 카테고리코드
	 */
	public BusinessUser(String userId, String userPw, String userName, String phoneNumber, String categoryCode) {
		super(userId, userPw, userName, phoneNumber, categoryCode);

	}
	/**
	 * BusinessUser 클래스의 기본 생성자
	 */
	public BusinessUser() {
		super();
	}
	...
	/**
	 * 기업 회원 정보를 담을 ArrayList
	 */
	public static List<BusinessUser> businessUserList = new ArrayList<>();
	...
	public void AccountRegister() {
		setCategoryCode(UserCategory.BUSINESS_CODE);
			...
			while (true) {
				...
				if (SignInValidator.BusinessCheckId(input)) {
					setUserId(input);
					break;
	 ...
	 */
	@Override
	public PageResult accountEdit() { // 계정 정보 수정
		...
		try (BufferedReader reader = new BufferedReader(
				new FileReader(com.test.project.file.FileManager.BUSINESSUSER))) {
			String line;
...
//				String oldPhoneNumber = token[3];
				if (oldId.equals(pageResult.id)) {
					line = pageResult.id + "■" + newPassword + "■" + newName + "■" + newPhoneNumber + "■business";
			...
			e.printStackTrace();
			pageResult.nextPage = MenuNameManager.기업회원_정보수정;
			//return pageResult;
		}

		try (BufferedWriter writer = new BufferedWriter(new FileWriter(FileManager.BUSINESSUSER))) {
